...
Handles product add / edit / delete and the dashboard.
"""
...
def allowed_file(filename):
    """Return True only if the extension is in the allowed set."""
...
@login_required
def dashboard():
    """Admin home – show product list and stock summary."""
...
@login_required
def add_perfume():
    """Show form and process new perfume creation."""
    if request.method == 'POST':
        ...
        # Handle image upload
        image_filename = save_image(request.files.get('image')) or 'placeholder.svg'
...
def edit_perfume(product_id):
    """Load existing perfume into a form and update it on POST."""
...
def delete_perfume(product_id):
    """Delete a perfume (POST only to prevent accidental deletion via GET)."""
    perfume = Perfume.query.get_or_404(product_id)
    name = perfume.name
...
def toggle_featured(product_id):
    """Quick toggle to feature/unfeature a product on the home page."""
    perfume = Perfume.query.get_or_404(product_id)
