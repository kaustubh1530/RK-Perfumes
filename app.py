...
def init_db_now():
    ...
    if not Admin.query.first():
        admin = Admin(
            email='admin@rkperfume.com',
            password_hash=generate_password_hash('RKPerfume@2025')
        )
        db.session.add(admin)
    if not Perfume.query.first():
        samples = [
            ...
            Perfume(name='RK Aventus Cologne', inspired_by='Creed - Aventus Cologne', description='Inspired by Creed Aventus Cologne. Fresh icy mint and pink pepper burst over clean woody base.', price=3499, quantity=18, category='Fresh', image_filename='placeholder.png', is_featured=True),
            Perfume(name='RK Black Phantom', inspired_by='Kilian - Black Phantom', description='Inspired by Kilian Black Phantom. Dark rum and black sugar envelop a smoky coffee heart.', price=3699, quantity=10, category='Oriental', image_filename='placeholder.png'),
        ...
        db.session.bulk_save_objects(samples)
        db.session.commit()
    return 'Done! Database initialized. Now DELETE this route!'
